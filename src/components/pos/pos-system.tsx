...
export function POSSystem({ businessId }: POSProps) {
  ...
  const handleWhatsAppShare = () => {
    if (!lastInvoice || !businessId) return;
    const invoiceId = (lastInvoice as { _id?: string })?._id;
    const invoiceNumber = (lastInvoice as { invoiceNumber?: string })?.invoiceNumber;
    const customerPhone = (lastInvoice as { customerPhone?: string })?.customerPhone;

    const businessName = (lastInvoice as { businessName?: string })?.businessName;
    const customerName = (lastInvoice as { customerName?: string })?.customerName;
    const total = (lastInvoice as { total?: number })?.total;
    const paymentMethod = (lastInvoice as { paymentMethod?: string })?.paymentMethod;

    const invoiceUrl = `${window.location.origin}/en/invoice/${businessId}/${invoiceId}`;

    const lines = [
      `*${businessName ?? "Your Store"}*`,
      ``,
      `Thank you${customerName && customerName !== "Walk-in Customer" ? `, ${customerName}` : ""}! 🙏`,
      `Here is your invoice summary:`,
      ``,
      `🧾 Invoice No: *${invoiceNumber}*`,
      total !== undefined ? `💰 Amount Paid: *${formatCurrency(total)}*` : null,
        ...
        {showSuccess && (
          ...
          >
            <motion.div
              ...
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.95, opacity: 0 }}
              className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-md text-center p-8"
            >
              <motion.div
                initial={{ scale: 0 }}
                animate={{ scale: 1 }}
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2.5 py-2.5 px-4 rounded-xl border-2 border-[#25D366] text-[#25D366] hover:bg-[#25D366]/10 transition-colors font-medium mb-4"
              >
                <svg
                  viewBox="0 0 24 24"
